<?php
	...
	if(isset($_POST['bsimpan']))
	{
		...
		{
			//Data akan di edit
			$edit = mysqli_query($host, "UPDATE jadwal_ti set
											 	hari = '$_POST[hri]',
											 	dosen = '$_POST[dsn]',
												matakuliah = '$_POST[mk]',
											 	waktu = '$_POST[time]',
		...
		else
		{
			...
			$simpan = mysqli_query($host, "INSERT INTO jadwal_ti (hari,dosen, matakuliah, waktu, tempat)
										  VALUES ('$_POST[hri]', 
										  		 '$_POST[dsn]', 
										  		 '$_POST[mk]', 
										  		 '$_POST[time]',
			...
			else
			{
				echo "<script>
						...
						document.location='jadwalTI.php';
				     </script>";
			}
		}
	...
	if(isset($_GET['hal']))
	{
		...
		if($_GET['hal'] == "edit")
		{
			...
			$tampil = mysqli_query($host, "SELECT * FROM jadwal_ti WHERE id_jadwal = '$_GET[id]'");
		...
		else if ($_GET['hal'] == "hapus")
		{
			...
			$hapus = mysqli_query($host, "DELETE FROM jadwal_ti WHERE id_jadwal = '$_GET[id]' ");
			if($hapus){
				echo "<script>
...
?>
                    <tr>
                        ...
                        <td><?=$data['tempat']; ?></td>
                        <td>
                            <a href="jadwaTI.php?hal=edit&id=<?=$data['id_jadwal']?>" class="btn btn-warning"> Edit </a>
                            <a href="jadwaTI.php?hal=hapus&id=<?=$data['id_jadwal']?>" 
